...
import { Users, Palette, FolderOpen, Calendar, TrendingUp, Activity, Shield, ShieldCheck, Trash2, Database, Loader2 } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const importCatalogMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest("POST", "/api/admin/import-catalog", {});
    },
    onSuccess: (response: any) => {
      ...
      toast({
        ...
        description: response?.data?.message || `Catalog contains ${response?.data?.count || 0} paints`,
      });
    },
    onError: (error: any) => {
      toast({
          ...
          <CardContent>
            <div className="space-y-4">
              <p className="text-gray-300">
                Manually sync the paint catalog from the WarHub community catalog. Syncing only adds new paints
                or updates catalog details; it does not remove paints, barcode links, or user inventory.
              </p>
